...
func (tray *Tray) setup() error {
	...
	tray.SetToolTip(l18n.Sprintf("DementiaWG: Deactivated"))
	...
	for _, item := range [...]struct {
		...
		{separator: true},
		{label: l18n.Sprintf("&О DementiaWG…"), handler: tray.onAbout, enabled: true},
			...
			if tunnelAction != nil {
				...
				case manager.TunnelStarted:
					if !wasChecked {
						...
						tray.ShowCustom(l18n.Sprintf("DementiaWG Activated"), l18n.Sprintf("The %s tunnel has been activated.", tunnel.Name), icon)
					}

				case manager.TunnelStopped:
					if wasChecked {
						icon, _ := loadSystemIcon("imageres", -31, 128) // TODO: this icon isn't very good...
						tray.ShowCustom(l18n.Sprintf("DementiaWG Deactivated"), l18n.Sprintf("The %s tunnel has been deactivated.", tunnel.Name), icon)
					}
				}
			}
		} else if !tray.mtw.Visible() {
			tray.ShowError(l18n.Sprintf("DementiaWG Tunnel Error"), err.Error())
		}
...
func (tray *Tray) updateGlobalState(globalState manager.TunnelState) {
	...
	tray.SetToolTip(l18n.Sprintf("DementiaWG: %s", textForState(globalState, true)))
